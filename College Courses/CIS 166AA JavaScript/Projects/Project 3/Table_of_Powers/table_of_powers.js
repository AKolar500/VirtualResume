...
var finding_powers = function() {
	//declare vars
	var count = 1, square, cube, number;
	//take the input from the user
	number = parseInt( $("integer").value );
	
	//create the string that will go into the large label
	$("total").value = "Digit" + "\t" + "Square" + "\t" + "Cube" + "\n";
	do {
		//calculate the square and the cube of the current value
		square = count * count;
		cube = count * count * count;
		//add the square and cube to the "total" string
		$("total").value += count + "\t" + square + "\t" + cube + "\n";
		//increment so the loop is not infinite
		count++;
	} while ( count <= number )
}
