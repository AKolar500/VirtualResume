...
var SlideShow = function ( params ) {
    ...
    this.imageMousedown = function (evt) {
        if (evt.button == 0 ) {
			var shiftIsDown;
			//This works in earlier browsers but not later versions
			if ("getModifierState" in evt) {
				if (evt.getModifierState("Shift")) {
					shiftIsDown = true;
				}
			//If the above doesn't work this is the fix for new browser versions
			} else {
	...
	//make the control key combined with the mouse reset the slide show, just like the escape key.
    this.imageControldown = function (evt) {
        if (evt.button == 0 ) {
			var controlIsDown;
			//This works in earlier browsers but not later versions
			if ("getModifierState" in evt) {
	...
	//new event handler
	jsLib.event.add ( this.imageNode, "mousedown", this.imageControldown );
