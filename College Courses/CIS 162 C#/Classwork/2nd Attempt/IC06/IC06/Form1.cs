...
namespace IC06
{
    public partial class Form1 : Form
    {
        ...
        const decimal WARRANTY_1_YEAR = 3000;
        const decimal WARRANTY_3_YEARS = 5000;
        ...
        private void btnCalculate_Click(object sender, EventArgs e)
        {
           ...
           // decimal decDummy;
            ...
            if (ValidateData())
            {
                //Operations are covered in ValidateData()
                strModel = txtModel.Text;
                strYear = txtYear.Text;
                decBasePrice = Convert.ToDecimal(txtBasePrice.Text);

                //call the TotalOptions method, then convert it to a currency string.
                decTotalOptions = TotalOptions();
                lblTotalOptions.Text = decTotalOptions.ToString("c");

                //call the ExtendedWarranty method, then convert it to a currency string.
                decExtendedWarranty = ExtendedWarranty();
                lblExtendedWarranty.Text = decExtendedWarranty.ToString("c");


                TaxAndLicensing(decBasePrice, out decStateAndLocalTaxes, out decFederalTaxes,
                    out decLicensing);
        ...
        private static void TaxAndLicensing(decimal xxx, out decimal decStateAndLocalTaxes,
        ...
        private decimal ExtendedWarranty()
        {
            ...
            if (rad1Year.Checked)
            {
                decExtendedWarranty = WARRANTY_1_YEAR;
            }
            else if (rad3Years.Checked)
            {
                decExtendedWarranty = WARRANTY_3_YEARS;
            }
            else
            {
